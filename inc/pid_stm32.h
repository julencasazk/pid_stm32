...
typedef struct {
    ...
    pid_float u_prev_unsat; // Needed for bumpless transfer
    uint8_t bumpless_pending; // Flag to hold step while trasnferring gains
 ...
 *
 * @param 	pid_handle: 	already created PID Object
 * @param 	kp: 			proportional g_PID_MODE_AUTOain
 * @param 	ki: 			integral gain
 * @param 	kd: 			derivative gain
 * @param 	N: 				derivative low-pass filter constant, inverse to time constant tau N = 1/tau where s = 1/(1+tau*s)
 * @param 	Ts: 			sample time, controller supposes its constant, like it should for discrete systems
...
/*
 * @brief 	Set PID mode, (disable or enable PID) 
 *
 * @param 	pid_handle:		    already created PID Object
 * @param 	mode:			PID operating mode, can have the following values:
 *                                      @arg    _PID_MODE_MANUAL
 *                                      @arg    _PID_MODE_AUTO
 *
 * @retVal:					PID error code
*/
...
/*
 * @brief 	Set smooth manual-to-auto transfer toggle
 *
 * @param 	pid_handle:		    already created PID Object
 * @param 	smooth:			    smooth manual-to-auto selector:
 *                                      @arg    _PID_SMOOTH_MANUAL_TO_AUTO_OFF
 *                                      @arg    _PID_SMOOTH_MANUAL_TO_AUTO_ON
 *
 * @retVal:					PID error code
*/
...
/*
 * @brief 	Set smooth tuning toggle
 *
 * @param 	pid_handle:		    already created PID Object
 * @param 	smooth:			    smooth tuning selector:
 *                                      @arg    _PID_SMOOTH_TUNING_OFF
 *                                      @arg    _PID_SMOOTH_TUNING_ON
 *
 * @retVal:					PID error code
*/
