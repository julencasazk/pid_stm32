...
static inline pid_float pid_integral_increment(pid_controller_t* pid_handle, pid_float e, pid_float e_prev)
{
	...
	switch (pid_handle->i_method) {
		case _PID_DISCRETE_FORWARD_EULER:
			return  Ki * Ts * e_prev;
		case _PID_DISCRETE_BACKWARD_EULER:
			return Ki * Ts * e;
		case _PID_DISCRETE_TUSTIN:
		default:
...
pid_err_t pid_init(pid_controller_t* pid_handle, pid_float kp, pid_float ki, pid_float kd, pid_float N, pid_float Ts)
{
    ...
    if (err != _PID_OK) return err;

    pid_handle->anti_windup = _PID_ANTI_WINDUP_OFF;
    pid_handle->derivative_on_meas = _PID_DERIVATIVE_ON_MEASURE_OFF;
    pid_handle->proportional_on_meas = _PID_PROPORTIONAL_ON_MEASURE_OFF;
    pid_handle->mode = _PID_MODE_AUTO;
    pid_handle->smooth_manual_to_auto = _PID_SMOOTH_MANUAL_TO_AUTO_ON;
    pid_handle->smooth_tuning = _PID_SMOOTH_TUNING_ON;
...
{
    if (pid_handle == NULL) return _PID_ERR_INVALID_ARGS;
...
pid_err_t pid_set_params(pid_controller_t* pid_handle, pid_float kp, pid_float ki, pid_float kd, pid_float N, pid_float Ts, pid_float kb_aw)
{
	...
	if (pid_handle->smooth_manual_to_auto || pid_handle->smooth_tuning)
		pid_handle->bumpless_pending = 1; // Bumpless transfer takes 2 steps in total
...
pid_float pid_run(pid_controller_t* pid_handle, pid_float setpoint, pid_float measurement)
{
	...
	if(pid_handle == NULL) return _PID_ERR_INVALID_ARGS;
	if(pid_handle->mode == _PID_MODE_MANUAL) return _PID_ERR_MANUAL_MODE_ON;

	pid_float e = setpoint - measurement;


	pid_float u_p = (pid_handle->proportional_on_meas ==  _PID_PROPORTIONAL_ON_MEASURE_ON)
					? pid_handle->kp * measurement
					: pid_handle->kp * e;


	pid_float w = (pid_handle->derivative_on_meas == _PID_DERIVATIVE_ON_MEASURE_ON)
					? -measurement
					: e;
    ...
    if (pid_handle->bumpless_pending)
    {
        pid_float u_target = pid_handle->u_prev_unsat;
        pid_handle->i_state = u_target - u_p - u_d;
        pid_handle->bumpless_pending = 0;

        pid_float u_unsat = u_p + pid_handle->i_state + u_d;
        pid_float u = clip(u_unsat, pid_handle->min_output, pid_handle->max_output);

        pid_handle->e_prev = e;
        pid_handle->u_prev = u;
        pid_handle->u_prev_unsat = u_unsat;
        return u;
    }
    ...
    switch (pid_handle->anti_windup) {
        case _PID_ANTI_WINDUP_INTEGRATOR_CLAMPING:
        {
...
{
    if (pid_handle == NULL ||
        derivative_on_meas < _PID_DERIVATIVE_ON_MEASURE_OFF ||
        derivative_on_meas > _PID_DERIVATIVE_ON_MEASURE_ON)
        return _PID_ERR_INVALID_ARGS;
...
{
    if (pid_handle == NULL ||
        proportional_on_meas < _PID_PROPORTIONAL_ON_MEASURE_OFF ||
        proportional_on_meas > _PID_PROPORTIONAL_ON_MEASURE_ON)
        return _PID_ERR_INVALID_ARGS;
...
{
	if (pid_handle == NULL ||
		anti_windup < _PID_ANTI_WINDUP_OFF ||
		anti_windup > _PID_ANTI_WINDUP_BACK_CALCULATION)
		return _PID_ERR_INVALID_ARGS;
...
pid_err_t pid_set_mode( pid_controller_t* pid_handle, PID_mode_TypeDef mode )
{
	if (pid_handle == NULL ||
		mode < _PID_MODE_MANUAL ||
		mode > _PID_MODE_AUTO)
		return _PID_ERR_INVALID_ARGS;

	PID_mode_TypeDef prev = pid_handle->mode;
	pid_handle->mode = mode;

	if (pid_handle->smooth_manual_to_auto &&
		prev == _PID_MODE_MANUAL &&
		mode == _PID_MODE_AUTO)
	{
		pid_handle->bumpless_pending = 1;
	}

	return _PID_OK;
}

pid_err_t pid_set_smooth_manual_to_auto( pid_controller_t* pid_handle,
                                        PID_smooth_manual_to_auto_TypeDef smooth )
{
	if (pid_handle == NULL ||
		smooth < _PID_SMOOTH_MANUAL_TO_AUTO_OFF ||
		smooth > _PID_SMOOTH_MANUAL_TO_AUTO_ON)
		return _PID_ERR_INVALID_ARGS;

	pid_handle->smooth_manual_to_auto = smooth;
	return _PID_OK;
}

pid_err_t pid_set_smooth_tuning( pid_controller_t* pid_handle,
                                PID_smooth_tuning_TypeDef smooth )
{
	if (pid_handle == NULL ||
		smooth < _PID_SMOOTH_TUNING_OFF ||
		smooth > _PID_SMOOTH_TUNING_ON)
		return _PID_ERR_INVALID_ARGS;

	pid_handle->smooth_tuning = smooth;
	return _PID_OK;
}
